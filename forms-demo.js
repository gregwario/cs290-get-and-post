...
app.get("/get-and-post", (request, response) => {
  // console.log(request.query, 'received get');
  var context = {};
  context.dataList = [];
  for (let item in request.query) {
    context.dataList.push({name: item, value: request.query[item]});
  }
  response.render('get-loopback-improved', context);
});

app.post("/get-and-post", (request, response) => {
  // console.log(request.body, 'received post');
  var context = {};
  context.dataList = [];
  for (let item in request.body) {
    context.dataList.push({name: item, value: request.body[item]});
  }
